import { FileText, Plus, Video, Play, Square, Edit2, Trash2, Clock } from "lucide-react";
...
export default function ConsultationHistory({
  ...
  const consultationsWithSynopsis = consultations.filter(c => c.synopsis);
  ...
  return (
    <div className="flex-1 flex overflow-hidden">
                ...
                return (
                  ...
                  >
                    <div>
                      ...
                      {consultation.notes && (
                        <p className="text-sm text-neutral-600 mt-3 line-clamp-2">{consultation.notes}</p>
                      )}
                    </div>
      ...
      {/* Right — patient history synopsis timeline */}
      <div className="w-80 flex-shrink-0 flex flex-col bg-neutral-50 overflow-hidden">
        <div className="px-5 py-5 border-b border-neutral-200 flex-shrink-0">
          <h3 className="text-sm font-semibold text-neutral-800">Patient History</h3>
          <p className="text-xs text-neutral-400 mt-0.5">Quick glance before diving in</p>
        </div>

        <div className="flex-1 overflow-y-auto px-4 py-4">
          {consultationsWithSynopsis.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center px-4 gap-2">
              <Clock size={28} className="text-neutral-300" />
              <p className="text-xs font-medium text-neutral-400">No summaries yet</p>
              <p className="text-[11px] text-neutral-400 leading-relaxed">
                Summaries appear here after a consultation is completed with at least one recording or file.
              </p>
            </div>
          ) : (
            <div className="relative">
              ...
              <div className="space-y-5">
                {consultationsWithSynopsis.map((c) => (
                  <button
                    key={c.id}
                    onClick={() => onSelect(c)}
                    className="w-full text-left pl-6 relative group"
                  >
                    {/* dot */}
                    <div className="absolute left-0 top-1.5 w-3.5 h-3.5 rounded-full border-2 border-neutral-300 bg-neutral-50 group-hover:border-teal-500 group-hover:bg-teal-50 transition-colors" />
                    ...
                    <p className="text-xs text-neutral-700 leading-relaxed group-hover:text-teal-800 transition-colors">
                      {c.synopsis}
                    </p>
                  </button>
                ))}
